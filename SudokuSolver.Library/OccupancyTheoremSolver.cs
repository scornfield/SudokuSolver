...
{
    // The occupancy theorem works based preemtive sets.  For any given group, if there are [n] tiles that share the same [n] possible values, 
    // and these are the only possible values for any of these tiles, then no other tile in the group can be any of those values, so we remove those as possibilities.
